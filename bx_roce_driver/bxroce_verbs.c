...
static int bxroce_prepare_send_wqe(struct bxroce_qp *qp, struct bxroce_wqe *tmpwqe, struct ib_send_wr *wr, int i)
{
...
static int bxroce_prepare_write_wqe(struct bxroce_qp *qp, struct bxroce_wqe *tmpwqe, struct ib_send_wr *wr, int i)
{
	int status;
...
static void bxroce_printk_wqe(struct bxroce_wqe *tmpwqe)
{
	BXROCE_PR("bxroce: ---------------check  wqe--------------\n");//added by hs
	...
	BXROCE_PR("bxroce:----------------check  wqe end------------\n");//added by hs
}
...
static int bxroce_build_sges(struct bxroce_qp *qp, struct bxroce_wqe *wqe, int num_sge, struct ib_sge *sg_list,struct ib_send_wr *wr)
{
	...
	//clear wqe before use it
	
	for (i = 0; i < num_sge; i++) {	

		//clear wqe before we use it
		memset(tmpwqe,0,sizeof(*tmpwqe));
		//prepare wqe ,need to verify ud/rc/rd/uc.
...
static int bxroce_build_inline_sges(struct bxroce_qp *qp, struct bxroce_wqe *wqe, const struct ib_send_wr *wr, u32 wqe_size)
{
	...
	if (wr->send_flags & IB_SEND_INLINE && qp->qp_type != IB_QPT_UD) {//
		...
		wqe_size +=((wr->num_sge-1) * sizeof(struct bxroce_wqe)); // useless now, del later.
	}
...
static void bxroce_ring_sq_hw(struct bxroce_qp *qp, const struct ib_send_wr *wr) {
	...
	{
		printk("send err!, nothing to send\n");
		return;
	}
...
static void bxroce_update_sq_tail(struct bxroce_dev *dev,struct bxroce_qp *qp)
{
		...
		bxroce_mpb_reg_write(dev,base_addr,PGU_BASE,WRITEORREADQPLIST,0x0);
		 ...
		 mutex_unlock(&dev->hw_lock);
		qp->sq.tail = tail / qp->sq.entry_size;

		if(qp->sq.tail == qp->sq.head)
		{
			qp->sq.qp_foe = BXROCE_Q_EMPTY;
		}
		BXROCE_PR("bxroce:sq.tail : 0x%x ,qp.head: 0x%x\n",qp->sq.tail,qp->sq.head);
}
...
static void bxroce_update_sq_head(struct bxroce_dev *dev, struct bxroce_qp *qp, const struct ib_send_wr *wr)
{
	...
	if(wr->num_sge){
		...
		else if((wr->opcode == IB_WR_RDMA_WRITE_WITH_IMM)||(wr->opcode == IB_WR_SEND_WITH_IMM)){
...
int bxroce_post_send(struct ib_qp *ibqp,const struct ib_send_wr *wr,const struct ib_send_wr **bad_wr)
{
	...
	while(wr){ //if UD, should support SEND OR SEND WITH IMM,or it can't do anything.
		...
		//update sq's head
		bxroce_update_sq_head(dev,qp,wr);
		wr = wr->next;
	}
...
static void bxroce_update_rq_head(struct bxroce_dev *dev, struct bxroce_qp *qp, const struct ib_recv_wr *wr)
{
	#if 0
	u32 qpn;
	u32 head;
...
static int bxroce_poll_hwcq(struct bxroce_cq *cq, int num_entries, struct ib_wc *ibwc)
{
		...
		printk("trpcqe'size:0x%x \n",sizeof(struct bxroce_txcqe));
		printk("rxrpcqe's size: 0x%x \n",sizeof(struct bxroce_rxcqe));
		printk("xmitrpcqe's size : 0x%x \n",sizeof(struct bxroce_xmitcqe));
			       ...
			       struct ib_udata *udata)
{
		...
		struct qp_change_info *qp_change_info =NULL;
		int status;
		u32 qp_num = 0;
		int sq_size;
		int rq_size;
		      ...
		      int attr_mask)
{
		...
		if(qp->qp_state == BXROCE_QPS_RTR)//now we may have got dest qp.we should map destqp and srcqp.
		{
			...
			if(qp->init_rqpsn)
			{
				...
				psn = (psn_8 << 24) | (qp->init_rqpsn & 0xffffff);
